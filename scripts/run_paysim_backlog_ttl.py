...
def _cohort_rows(future, cycles, selected, review_step, ttl_name: str):
    rows = []
...
def _validate_v13_infinite(frontier: pd.DataFrame, reference_path: Path) -> bool:
    ...
    infinite = frontier.loc[frontier.ttl_steps == "infinite"]
    if len(infinite) != 1:
        raise ValueError("Expected one infinite TTL row")
    ...
    for column in (
        ...
        if not np.isclose(float(left[column]), float(right[column]), rtol=0.0, atol=1e-12):
            raise AssertionError(f"Infinite TTL failed to reproduce v1.13 column {column}")
    return True
...
def main() -> None:
    ...
    parser.add_argument("--out", default="results/paysim_backlog_ttl")
    parser.add_argument(
        "--handoff-reference",
    ...
    work = out_dir / "paysim_backlog_ttl_features.parquet"
    db = out_dir / "paysim_backlog_ttl.duckdb"
    ...
    del validation
    if len(cycles) != 3:
        raise ValueError("Canonical TTL frontier expects three rolling cycles")
    ...
    for ttl in TTL_GRID:
        name = ttl_label(ttl)
        selected, review_step, review_regime, expired, schedule, refresh = (
            continuous_rescore_backlog_ttl(
                future_step,
                future_key,
                regime_by_step,
        ...
        expiry = expiry_metrics(y, amount, expired)
        ...
        peak_pending = int(schedule.pending_after_selection.max())
        metric_rows.append(
            {
                "ttl_steps": name,
                "ttl_numeric": np.inf if ttl is None else int(ttl),
                "profile": PROFILE,
                **metrics,
                **expiry,
                ...
                "max_pending_age_observed_steps": int(schedule.pending_age_max_steps.max()),
            }
        )
    ...
    infinite = frontier.loc[frontier.ttl_steps == "infinite"].iloc[0]
    ...
    frontier = frontier.sort_values("ttl_numeric")
    frontier.to_csv(out_dir / "ttl_frontier.csv", index=False)

    pd.concat(schedule_frames, ignore_index=True).to_csv(
        out_dir / "ttl_capacity_schedule.csv", index=False
    )
    pd.concat(refresh_frames, ignore_index=True).to_csv(
        out_dir / "ttl_refresh_workload.csv", index=False
    )
    pd.DataFrame(cohort_rows).to_csv(out_dir / "ttl_arrival_cohort_metrics.csv", index=False)
    ...
    for name, mask in masks.items():
        ...
        overlap_rows.append(
            {
                "reference_ttl": "infinite",
                "ttl_steps": name,
                **queue_overlap(masks["infinite"], mask),
            }
        )
    pd.DataFrame(overlap_rows).to_csv(out_dir / "ttl_queue_overlap.csv", index=False)

    entitlement = int(np.floor(REFERENCE_CAPACITY * len(future) / 10_000))
    if not (frontier.alerts == entitlement).all():
        raise AssertionError("Every TTL must consume the same continuous final capacity")
    ...
    summary = {
        ...
        "ttl_grid": [ttl_label(value) for value in TTL_GRID],
        ...
        "operational_candidate_ttls": candidate_rows.ttl_steps.tolist(),
        "runtime_seconds": float(time.time() - started),
        "interpretation_boundaries": [
            "TTL is applied only to seen-but-unreviewed cases; fraud labels do not drive expiry or queue selection.",
            "A case is eligible at step t when t-arrival_step is no greater than the declared TTL. Expiry occurs before refresh-time rescoring, so finite TTL can reduce release workload.",
            "All TTLs use the same balanced rolling systems, rescore-on-refresh handoff rule, continuous 50 reviews/10k entitlement and deterministic non-label tie-break.",
            "The predeclared detection guardrail is no worse than 2 percentage points versus infinite TTL separately for precision, fraud recall and fraud-value recall. The workload target is at least 50% fewer refresh-time rescores.",
            "The operational_candidate field is a deterministic frontier screen, not a statistical promotion decision and not a production recommendation.",
            "The infinite TTL must reproduce the v1.13 rescore_pending result when the main-branch reference file is available.",
            "PaySim is synthetic; dataset steps are not production SLA units, transaction amount is not prevented loss, and the upstream rolling lifecycle still assumes prior-period labels are available at refresh."
        ],
    }
